...
var { toRegex, makeRe } = require('./dist');
...
describe('to-regex', function() {
  ...
  describe('.makeRe', function() {
    it('should be a function', function() {
      assert.equal(typeof makeRe, 'function');
    });

    it('should create a strict regex from the given string', function() {
      assert.deepEqual(makeRe('foo'), /^(?:foo)$/);
    });

    it('should return a regex unchanged', function() {
      assert.deepEqual(makeRe(/whatever/), /whatever/);
    });

    it('should create a loose regex when `options.contains` is true', function() {
      assert.deepEqual(makeRe('foo', {contains: true}), /(?:foo)/);
    });

    it('should create a negation regex when `options.negate` is true', function() {
      assert.deepEqual(makeRe('foo', {negate: true}), /^(?:^(?:(?!^(?:foo)$).)+$)$/);
      assert.equal(makeRe('foo', {negate: true}).test('foo'), false);
      assert.equal(makeRe('foo', {negate: true}).test('bar'), true);
      assert.equal(makeRe('foo', {negate: true}).test('foobar'), true);
      assert.equal(makeRe('foo', {negate: true}).test('barfoo'), true);
    });

    it('should create a loose negation regex when `options.strictNegate` is false', function() {
      assert.deepEqual(makeRe('foo', {strictNegate: false}), /^(?:^(?:(?!(?:foo)).)+$)$/);
      assert.equal(makeRe('foo', {strictNegate: false}).test('foo'), false);
      assert.equal(makeRe('foo', {strictNegate: false}).test('bar'), true);
      assert.equal(makeRe('foo', {strictNegate: false}).test('foobar'), false);
      assert.equal(makeRe('foo', {strictNegate: false}).test('barfoo'), false);
    });

    it('should create a loose negation regex when `options.contains` and `options.negate` are true', function() {
      assert.deepEqual(makeRe('foo', {contains: true, negate: true}), /^(?:^(?:(?!(?:foo)).)+$)$/);
      assert.equal(makeRe('foo', {contains: true, negate: true}).test('foo'), false);
      assert.equal(makeRe('foo', {contains: true, negate: true}).test('bar'), true);
      assert.equal(makeRe('foo', {contains: true, negate: true}).test('foobar'), false);
      assert.equal(makeRe('foo', {contains: true, negate: true}).test('barfoo'), false);
    });

    it('should not enforce beginning anchor when `options.strictOpen` is false', function() {
      assert.deepEqual(makeRe('foo', {strictOpen: false}), /(?:foo)$/);
    });

    it('should not enforce ending anchor when `options.strictClose` is false', function() {
      assert.deepEqual(makeRe('foo', {strictClose: false}), /^(?:foo)/);
    });

    it('should use flags passed on `options.flags`', function() {
      assert.deepEqual(makeRe('foo', {flags: 'ig'}), /^(?:foo)$/ig);
    });
    ...
    it('should not cache regex when options.cache is false', function() {
      makeRe('whatever', {cache: false});
      makeRe('whatever', {cache: false});
      makeRe('whatever', {cache: false});
      var re = makeRe('whatever', {cache: false});
    ...
    it('should add `i` to flags when `options.nocase` is true', function() {
      assert.deepEqual(makeRe('foo', {nocase: true}), /^(?:foo)$/i);
    });
